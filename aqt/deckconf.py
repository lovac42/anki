...
class DeckConf(QDialog):
    def __init__(self, mw, deck):
        ...
        self.mw = mw
        self.deck = deck
        self.childDids = self.mw.col.decks.get(self.deck.getId()).getDescendantsIds()
    ...
    def parentLimText(self, type="new"):
        ...
        if "::" not in self.deck['name']:
            return ""
        lim = -1
        for ancestor in self.deck.ancestors():
            conf = self.mw.col.decks.get(ancestor['id']).getConf()
    ...
    def loadConf(self):
        self.conf = self.mw.col.decks.get(self.deck.getId()).getConf()
        # new
        conf = self.conf['new']
