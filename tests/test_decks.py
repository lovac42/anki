...
def test_basic():
    ...
    assert deck.decks.get(1).getName()
    # create a new deck
...
def test_remove():
    ...
    assert str(deck.decks.get(c.did).getId()) == "1"
...
def test_renameForDragAndDrop():
    ...
    HSK = d.decks.byName("HSK", create=True)
    assertException(DeckRenameError, lambda: HSK.dragOnto(chinese))
    HSK.rem()

    # can't drack a deck where sibling have same name different case
    new_hsk = d.decks.byName("hsk", create=True)
    assertException(DeckRenameError, lambda: new_hsk.dragOnto(chinese))
    new_hsk.rem()

    # '' is a convenient alias for the top level DID
    hsk.dragoOnto(hsk_did, '')
