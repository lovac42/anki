...
def test_review_limits():
    ...
    assert parent.revCount() == 5 # parent
    assert child.revCount() == 5 # child
    ...
    assert parent.revCount() == 4
    assert child.revCount() == 4
    ...
    assert child.revCount() == 4 # child
    assert parent.revCount() == 9 # parent
...
def test_filt_reviewing_early_normal():
    ...
    assert d.decks.all(sort=True)[0].revCount() == 1
...
def test_deckDue():
    ...
    def l(i):
        d = cnts[i]
        return [d.getName(), d.getId(), d.revCount(), d.lrnCountSingle(), d.newCountSingle()]
    ...
    assert tree[0].revCount() == 1
    assert tree[0].newCount() == 1
    # child count is just review
    assert tree[0].getChildren()[0].getBaseName() == "1"
    assert tree[0].getChildren()[0].getId() == default1
    assert tree[0].getChildren()[0].revCount() == 1
    assert tree[0].getChildren()[0].newCount() == 0
