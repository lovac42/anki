...
class DeckManager:
    ...
    def id(self, name, create=True, type=None):
        ...
        for id, deck in list(self.decks.items()):
            if unicodedata.normalize("NFC", deck['name'].lower()) == name.lower():
                return int(id)
        if not create:
            return None
...
same id."""
    ...
    def remConf(self, id):
        ...
        for conf in self.all():
            # ignore cram decks
            if 'conf' not in conf:
                continue
            if str(conf['conf']) == str(id):
                conf['conf'] = 1
                self.save(conf)
    ...
    def children(self, did, includeSelf=False, sort=False):
        ...
        return [
            (deck['name'], deck['id'])
            for deck in self.childrenDecks(did, includeSelf=includeSelf, sort=sort)]
