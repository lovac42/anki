...
class FixingManager:
    ...
    def noteWithMissingModel(self):
        ...
        select id, flds, tags, mid from notes where mid not in """ + ids2str(self.col.models.ids()),
                  ...
                  lambda lines: self.col.remNotes([line[0]for line in lines], reason=f"Its note type is missing"
                  )
        )
    ...
    def req(self):
        for m in self.col.models.all():
            if m['type'] == MODEL_STD:
                ...
                if 'req' not in m:
                    self.col.models._updateRequired(m)
        ...
        for m in self.col.models.all():
            if m['type'] == MODEL_STD:
                self.template(
                    ...
                    lambda lines: self.col.remCards([line[0]for line in lines], reason=f"Cards removed because of missing templates."
                    )
                )
    ...
    def wrongNumberOfField(self):
        for m in self.col.models.all():
            ...
            if nids:
                self.col.remNotes([line[0]for line in lines],reason=f"It hads a wrong number of fields")
    ...
    def noteWithoutCard(self):
        ...
        if noteWithoutCard:
            ...
            for nid, flds, tags, mid in l:
                ...
                self.col._newCard(note,template0, self.col.nextID("pos"))
            ...
            self.template(
                ...
                lambda lines: self.col.remNotes([line[0]for line in lines],reason=f"It has no cards")
            )
    ...
    def cardWithoutNote(self):
        self.template(
            ...
            lambda lines: self.col.remCards([line[0]for line in lines],reason=f"Card's note is missing")
        )
    ...
    def intermediate(self):
        ...
        # field cache
        for m in self.col.models.all():
            self.col.updateFieldCache(self.col.models.nids(m))
