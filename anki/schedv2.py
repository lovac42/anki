...
and due < ?""" %
            self.col.decks._activeAsString(),
...
and due <= ?""" % self.col.decks._activeAsString(),
                                            self.today))
        # previews
        self.setLrnCount(self.lrnCount() + self.col.db.scalar(f"""
...
""" % self.col.decks._activeAsString()))
    ...
    def _todayLrnForDeck(self, deck):
        deck.count['singleDue']['lrn'] = deck.count['singleDue']['dayLrn'] + deck.count['singleDue']['todayNbCardLrn']
    # Reviews
    ##########################################################################
