...
class AddonManager:
    ...
    def install(self, file, manifest=None): # todo
    ...
    def _install(self, dir, zfile): #todo
        ...
        # extract
        for zip in zfile.namelist():
            if zip.endswith("/"):
                # folder; ignore
                continue

            path = os.path.join(base, zip)
            # skip existing user files
            if os.path.exists(path) and zip.startswith("user_files/"):
                continue
            zfile.extract(zip, base)
