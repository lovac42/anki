...
where card.nid = note.id and card.id in %s group by nid""" % ids2str(cids)):
    ...
    def _renderQA(self, data, cid=1, nid=1, did=1, tags="", cardFlags=0, qfmt=None, afmt=None):
        """Returns hash of id, question, answer.
        Keyword arguments:
        data -- [mid, ord, flds] (see db
        ...
        d = dict()
        d['id'] = cid
        d['q'] = self._renderQuestion(data, cid, nid, did, tags, cardFlags, fields, flds, ord, template, model, qfmt)
        d['a'] = self._renderAnswer(data, cid, nid, did, tags, cardFlags, fields, ord, template, model, afmt)
        return d
    ...
    def _renderQuestion(self, data, cid, nid, did, tags, cardFlags, fields, flds, ord, template, model, qfmt=None):
        ...
        question = self.__renderQA(fields, model, data, cid, nid, did, tags, cardFlags, format, "q")
    ...
    def _renderAnswer(self, data, cid, nid, did, tags, cardFlags, fields, ord, template, model, afmt=None):
        ...
        return self.__renderQA(fields, model, data, cid, nid, did, tags, cardFlags, format, "a")

    def __renderQA(self, fields, model, data, cid, nid, did, tags, cardFlags, format, type):
