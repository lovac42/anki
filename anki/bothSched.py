...
order by due""" % (self._deckLimit()),
    ...
    def _updateStats(self, card, type, cnt=1):
        key = type+"Today"
        for ancestor in card.deck.ancestors(True):
            # add
            ancestor[key][1] += cnt
    ...
    def _walkingCount(self, limFn=None, cntFn=None):
        ...
        for did in self.col.decks.active():
            ...
            if not lim:
                continue
            # check the parents
            ancestors = self.col.decks.get(did).ancestors()
...
did = ? and queue = {QUEUE_NEW_CRAM} limit ?)""", did, lim)
    ...
    def _deckNewLimit(self, did, fn=None):
        ...
        for ancestor in self.col.decks.get(did).ancestors(True)
            rem = fn(ancestor)
            if lim == -1:
                lim = rem
            else:
