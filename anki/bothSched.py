...
order by due""" % (self.col.decks._deckLimit()),
    ...
    def newCount(self):
        return self.col.decks.current().newCount()
    ...
    def _fillNew(self):
        ...
        while self._newDids:
            ...
            lim = min(self.queueLimit, deck.newLim())
            if lim:
    ...
    def lrnCount(self):
        return self.col.decks.current().lrnCount()
...
did = ? and queue = {QUEUE_DAY_LRN} and due <= ? limit ?""",
    ...
    def revCount(self):
        return self.col.decks.current().revCount()

    def revLim(self):
        return self.col.decks.current().revLim()
...
To study outside of the normal schedule, click the Custom Study button below."""))
    ...
    def allRevDue(self):
        ...
        return self.col.decks.current().allRevCount()

    def unseenDue(self):
        "True if there are any cards unseen in current deck."
        return self.col.decks.current().unseenCount()
