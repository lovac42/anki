...
class Deck(DictAugmentedDyn):
    ...
    def setNums(self):
        ...
        self.count['']['new'] = min(self.count['']['new'], self.newLim())
        ...
        self.count['']['rev'] =  min(self.count['']['rev'], self.revLim())
    ...
    def revCount(self):
        if self.resetted < NUMS:
            self.setNums()
        return self.count['']['rev']

    def allRevCount(self):
        if self.resetted < NUMS:
            self.setNums()
        return self.count['']['allRev']

    def unseenCount(self):
        if self.resetted < NUMS:
            self.setNums()
        return self.count['']['unseen']

    def lrnCount(self):
        if self.resetted < NUMS:
            self.setNums()
        return self.count['']['lrn']

    def newCountSingle(self):
        if self.resetted < NUMS:
            self.setNums()
        return self.count['single']['new']

    def revCountSingle(self):
        if self.resetted < NUMS:
            self.setNums()
        return self.count['single']['rev']

    def lrnCountSingle(self):
        if self.resetted < NUMS:
            self.setNums()
        return self.count['single']['lrn']

    def newLim(self):
        if self.resetted < LIMS:
            self.setLims()
        return self.count['lim']['new']

    def revLim(self):
        if self.resetted < LIMS:
            self.setLims()
        return self.count['lim']['rev']
