...
lapses=?, left=?, odue=?, odid=?, did=? where id = ?""",
    ...
    def _getQA(self, reload=False, browser=False):
        ...
        if not self._qa or reload:
            ...
            data = [self.id, note.id, model.getId(), self.originalDeck(), self.ord,
    ...
    def timeLimit(self):
        ...
        conf = self.col.decks.get(self.originalDeck()).getConf()
    ...
    def shouldShowTimer(self):
        ...
        According to the deck's information."""
        conf = self.col.decks.getConf(self.originalDeck())
        return conf['timer']
